        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 sm:mb-16">
          <div className="max-w-3xl space-y-3.5">
            <ScrollReveal effect="fade-down" delay={0}>
              <div className="inline-flex items-center gap-2 text-xs font-bold tracking-wider uppercase text-[#0284C7] bg-sky-50 px-3.5 py-1 rounded-full border border-sky-200/80 shadow-xs">
                <span className="w-2 h-2 rounded-full bg-sky-500 animate-ping" />
                <span>Operational Performance Benchmark</span>
              </div>
            </ScrollReveal>
            ...
            <ScrollReveal effect="fade-up" delay={60}>
              <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#0A2540] tracking-tight leading-tight text-balance">
                Elevating Cross-Border Trade Beyond Conventional Forwarding
              </h2>
            </ScrollReveal>

            <ScrollReveal effect="fade-up" delay={120}>
              <p className="text-sm sm:text-base lg:text-lg text-neutral-600 leading-relaxed max-w-2xl font-normal text-pretty">
                Discover how transitioning from fragmented legacy brokerage to our integrated statutory trade governance and direct carrier infrastructure drives measurable speed, fiscal recovery, and risk protection.
              </p>
            </ScrollReveal>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 items-start">
          {comparisonPoints.map((item, idx) => {
            ...
            return (
              ...
              >
                <div
                  onClick={() => setActiveHoverId(activeHoverId === item.id ? null : item.id)}
                  onMouseEnter={() => setActiveHoverId(item.id)}
                  onMouseLeave={() => setActiveHoverId(null)}
                  className={`rounded-3xl p-6 sm:p-7 transition-all duration-300 border flex flex-col justify-between h-full group relative overflow-hidden cursor-pointer select-none ${
                    isHovered
                      ? 'bg-white border-[#0284C7]/50 shadow-xl -translate-y-1'
                      : 'bg-white/90 hover:bg-white border-neutral-200/80 shadow-xs hover:border-sky-300 hover:shadow-md'
                  }`}
                >
                  {/* Top Ambient Highlight Border on Active/Hover */}
                  ...
                  <div>
                    {/* Top Row: Milestone Index + Category Pill + Benchmark Metric */}
                    <div className="flex items-center justify-between gap-3 mb-4">
                      <div className="flex items-center gap-2.5">
                        <span className="text-xs font-mono font-bold text-sky-700 bg-sky-50 px-2.5 py-1 rounded-md border border-sky-200/60">
                          {item.num}
                        </span>
                      ...
                      {/* Quantifiable Impact Metric Tag */}
                      <span className="inline-flex items-center gap-1 text-xs font-mono font-bold text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200 shadow-2xs">
                        <Zap className="w-3.5 h-3.5 text-emerald-600" />
                        <span>{item.metric}</span>
                      </span>
                    </div>

                    {/* Milestone Dimension Title */}
                    <div className="mb-2">
                      <h3 className="text-base sm:text-lg font-bold text-[#0A2540] group-hover:text-[#0284C7] transition-colors leading-snug">
                        {item.label}
                      </h3>
                    </div>

                    {/* Hover/Tap Reveal Trigger Prompt */}
                    <div className="flex items-center justify-between text-xs font-semibold text-neutral-400 group-hover:text-sky-600 transition-colors pt-1">
                      <span className="flex items-center gap-1.5">
                        ...
                        <span>Tap or hover to inspect comparison</span>
                      </span>
                    ...
                    >
                      <div className="overflow-hidden">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 pt-1 animate-in fade-in duration-300">
                          {/* Conventional Process (Rose Tinted) */}
                          <div className="p-4 rounded-2xl bg-rose-50/70 border border-rose-100 flex flex-col justify-between space-y-3">
                            <div>
                              <div className="flex items-center gap-1.5 text-[11px] font-mono font-bold uppercase text-rose-600 mb-1.5">
                                ...
                                <span>Conventional Process</span>
                              </div>
                          ...
                          {/* Modern SkyLink Approach (Emerald/Sky Tinted) */}
                          <div className="p-4 rounded-2xl bg-emerald-50/70 border border-emerald-200/90 shadow-xs flex flex-col justify-between space-y-3">
                            <div>
                              <div className="flex items-center gap-1.5 text-[11px] font-mono font-bold uppercase text-emerald-700 mb-1.5">
                                ...
                                <span>Modern SkyLink Approach</span>
                              </div>
